...
//
// HAND EDIT, KNOWINGLY. `dc-runtime/` is not in this repo, so that rebuild
// cannot be run here and this file is the only copy of the runtime we have.
// One section is edited by hand: `// src/cdn.ts`, so that React and ReactDOM
// load from `_vendor/react-18.3.1/` instead of unpkg.com and every `.dc.html`
// document opens with no network. Search this file for OFFLINE-FIRST to see
// the whole of it — it is ~20 lines in one place.
//
// If dc-runtime is ever rebuilt over this file, that edit is lost and the
// documents go back to needing the network. `qa/offline-documents.test.mjs`
// fails the moment that happens, and says what to put back.
"use strict";
(() => {
  ...
  // src/cdn.ts
  //
  // OFFLINE-FIRST. React and ReactDOM are vendored into `_vendor/react-18.3.1/`,
  // the same way the design system is vendored into `_ds/`, and are loaded from
  // there. A `.dc.html` document therefore opens on a plane, behind a corporate
  // proxy, and from a colleague's Downloads folder.
  //
  // The unpkg URLs stay for two reasons. They are the provenance of the
  // vendored bytes — each `_SRI` below is the sha384 of the file now sitting in
  // `_vendor/`, so anyone can prove the local copy is the published one — and
  // they remain the last resort if a checkout arrives without `_vendor/`.
  //
  // Babel is deliberately NOT vendored. It is only ever fetched to transpile a
  // `.jsx`/`.tsx` module named by an `<x-import>`, and no document in this repo
  // ships one, so vendoring its ~3 MB would buy nothing. If a document ever
  // does ship JSX, `qa/offline-documents.test.mjs` fails and says so.
  var REACT_URL = "https://unpkg.com/react@18.3.1/umd/react.production.min.js";
  var REACT_SRI = "sha384-DGyLxAyjq0f9SPpVevD6IgztCFlnMF6oW/XQGmfe+IsZ8TqEiDrcHkMLKI6fiB/Z";
  var REACT_LOCAL = "_vendor/react-18.3.1/react.production.min.js";
  var REACT_DOM_URL = "https://unpkg.com/react-dom@18.3.1/umd/react-dom.production.min.js";
  var REACT_DOM_SRI = "sha384-gTGxhz21lVGYNMcdJOyq01Edg0jhn/c22nsx0kyqP0TxaV5WVdsSH1fSDUf5YJj1";
  var REACT_DOM_LOCAL = "_vendor/react-18.3.1/react-dom.production.min.js";
  var BABEL_URL = "https://unpkg.com/@babel/standalone@7.29.0/babel.min.js";
  var BABEL_SRI = "sha384-m08KidiNqLdpJqLq95G/LEi8Qvjl/xUYll3QILypMoQ65QorJ9Lvtp2RXYGBFj1y";
  // `_vendor/` sits beside this file, so resolve against this file's own URL
  // rather than the document's. Works under http and under file://.
  var VENDOR_BASE = (() => {
  ...
  // `local` omitted → the old behaviour, which is what the remote retry and
  // Babel want. A host that supplies `window.__resources` still wins outright:
  // it is the editor telling us where it put the file.
  function cdnScriptFor(url, sri, local) {
    const res = window.__resources;
    const v = res ? res[url] : void 0;
    if (typeof v === "string" && v) return { src: v };
    if (local) return { src: VENDOR_BASE ? VENDOR_BASE + local : local };
  ...
  function loadReactUmd() {
    ...
    // OFFLINE-FIRST. Both tags carry `async = false`, so they execute in
    // insertion order and ReactDOM always sees React. A retry therefore
    // re-appends the pair rather than the half that failed.
    const pair = (fromVendor) => {
      const react = cdnScriptFor(REACT_URL, REACT_SRI, fromVendor && REACT_LOCAL);
      const reactDom = cdnScriptFor(REACT_DOM_URL, REACT_DOM_SRI, fromVendor && REACT_DOM_LOCAL);
      return Promise.all([
        loadScript(react.src, react.integrity),
        loadScript(reactDom.src, reactDom.integrity)
      ]);
    };
    return pair(true).catch((err) => {
      console.warn(
        "[dc] the vendored React in _vendor/react-18.3.1 did not load (" +
        err.message + "). Falling back to unpkg.com, so this document now " +
        "needs the network. Restore _vendor/ to make it open offline again."
      );
      return pair(false);
    }).then(() => void 0);
  }
